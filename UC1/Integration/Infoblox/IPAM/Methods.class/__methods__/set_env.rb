...
begin
  ...
  when 'vm'
    ...
    environment = prov.get_option("network") || prov.get_option(:network) || prov.get_option('dialog_network') || prov.get_option(:dialog_network) rescue nil
  when 'miq_provision'
    ...
    environment = ws_values[:network] || ws_values[:dialog_network] rescue nil
  else
