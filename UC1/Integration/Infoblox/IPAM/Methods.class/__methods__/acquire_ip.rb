...
begin
  ...
  def call_infoblox(action, ref, content_type, return_type, body = nil, return_fields = nil)
    ...
    if return_fields.nil?
      ...
      url = "#{@base_url}" + "#{ref}" + "\?_return_fields=#{return_fields}"
    end
  ...
  def parse_json_response(response)
    ...
    log(:info, "Running parse_json_response...#{response_hash}")
  ...
  def reserve_ip(hostname, network, dns_view, aliases = nil, start_ip = nil, end_ip = nil)
    begin
      ...
      host_response = call_infoblox(:post, 'record:host', :json, :json, body, 'ipv4addrs')
      log(:info, "Inspecting host: #{host_response}")

      # pull the ip from the host object
      host_hash = parse_json_response(host_response)
      ip_addr = host_hash["ipv4addrs"][0]["ipv4addr"]
  ...
  def set_prov(prov, hostname, ip_addr, net_mask, gateway, dns_domain, portgroup)
    log(:info, "Running set_prov...")
    log(:info, "GetIP --> Hostname = #{hostname}")
    log(:info, "GetIP --> IP Address =  #{ip_addr}")
