#
# Description: Method to get Service Owner
#
...
#app_name = $evm.root['dialog_snow_app_name_box']
hash["result"].each { |x| if x["name"] == app_name then sys_id= x["it_application_owner"]["value"] end}
$evm.log(:info, "BHP: ServiceNow API payload: SysID: #{sys_id}")
#API Call to get Service Owner
...
$evm.log(:info, "BHP: ServiceNow API payload: Service Owner Owner: #{data}")
values = {}
#values[nil] = "<Select>"
