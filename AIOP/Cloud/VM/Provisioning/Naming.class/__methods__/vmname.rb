          ...
          module Naming
            class VmName
              ...
              def derived_name
                ...
                elsif !supplied_name.present? && provision_object.get_option(:vm_suffix).nil?
                  [prefix, env_tag, suffix(false)]
                elsif !supplied_name.present? && !provision_object.get_option(:vm_suffix).nil?
                  [provision_object.get_option(:vm_name).to_s.strip, !provision_object.get_option(:vm_suffix)]
                end
              end
              ...
              def suffix(condensed)
                @handle.log(:info, "vm_name num_of_vms: #{provision_object.get_option(:num_of_vms)}")
                @handle.log(:info, "vm_name number_of_vms: #{provision_object.get_option(:number_of_vms)}")
                "$n{3}" if (provision_object.get_option(:number_of_vms) > 1 || !condensed) && provision_object.get_option(:vm_suffix).nil?
              end
            end
          end
        end
