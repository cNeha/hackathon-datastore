...
class LaunchAnsibleJob
  ...
  def machine_credential(job_template, credential_name)
    credential_id = nil
    credentials = job_template.manager.credentials
    credentials.each do |cred|
      if cred.name == credential_name and cred.type == "ManageIQ::Providers::AnsibleTower::AutomationManager::MachineCredential"
        credential_id = cred.manager_ref
      end
    end
    credential_id
  end
  ...
  def run(job_template, target)
    vm = @handle.vmdb(:vm).find_by_name(target) if target
    @handle.log(:info, "Processing Job Template #{job_template.name}")
    args = {:extra_vars => extra_variables}
    args[:limit] = target if target
    credential_id = machine_credential(job_template, vm.tags.find {|e| /^machine_credentials/ =~ e}[20..-1]) if target and !vm.tags.find {|e| /^machine_credentials/ =~ e}.nil?
    args[:credential] = credential_id unless credential_id.nil?
    @handle.log(:info, "target #{target}")
