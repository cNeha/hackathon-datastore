...
def set_vm_name(dialog_options_hash, prov)
    ...
    if !dialog_options_hash.fetch(:number_of_vms, nil).nil? && dialog_options_hash.fetch(:number_of_vms, nil) > 1
      new_vm_name << "-#{rand.to_s[2..6]}"
    end
  	log_and_update_message(:info, "number_of_vms: #{dialog_options_hash.fetch(:number_of_vms, nil)}", true)
    log_and_update_message(:info, "new_vm_name: #{new_vm_name}", true)
    if new_vm_name.blank?
      set_all_vm_name_attrs(prov, new_vm_name)
      log_and_update_message(:info, "Using default vm name: #{new_vm_name}", true)
    else
      set_all_vm_name_attrs(prov, new_vm_name)
      log_and_update_message(:info, "Setting vm name to: #{new_vm_name}", true)
    end
