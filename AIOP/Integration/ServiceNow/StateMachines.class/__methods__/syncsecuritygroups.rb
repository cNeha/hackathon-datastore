    ...
    module StateMachines
      class SyncSecurityGroups
        ...
        def main
          ...
          groups = []
          groups = @handle.vmdb(:security_group).all

          groups.each do |group|
            payload = build_payload(group)
            servicenow_sysid = get_system_id(group)
        ...
        def delete_groups
          ...
          servicenow_query.each do |result|
          	zone = @handle.vmdb(:security_group).find_by_id(result['u_id'])
            if zone.nil?
...
if __FILE__ == $PROGRAM_NAME
	Integration::ServiceNow::StateMachines::SyncSecurityGroups.new.main
end
