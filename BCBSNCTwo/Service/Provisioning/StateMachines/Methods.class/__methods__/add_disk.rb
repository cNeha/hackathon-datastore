...
#
# Notes: This method adds a disk to a VMWARE VM
#
#------------------------------------------------------------------------------
...
begin
  ...
  #------------------------------------------------------------------------------
  def call_vmware(servername, username, password, action, ref=nil, body_type=:xml, body=nil)
    #
    ...
    if @debug
      $evm.log(:info, "Calling VMWAREM at: #{url}")
  ...
  #
  # Extract the VMWARE-specific Storage Domain ID
  #
  ...
  unless storage_domain_id.nil?
    #
    # Extract the IP address and credentials for the VMWARE Provider
    #
    ...
    response = call_vmware(servername, username, password, :post, "#{vm.ems_ref}/disks", :xml, body)
    #
    ...
    while creation_status != "complete"
      ...
      if counter == 0
        ...
        sleep 5
        response = call_vmware(servername, username, password, :get, creation_status_href, :xml, nil)
        doc = Nokogiri::XML.parse(response.body)
    ...
    #
    response = call_vmware(servername, username, password, :get, disk_href, :xml, nil)
    doc = Nokogiri::XML.parse(response.body)
    if doc.at_xpath("/disk/active").text != "true"
      $evm.log(:info, "Activating disk")
      body = "<action/>"
      response = call_vmware(servername, username, password, :post, activate_href, :xml, body)
    else
